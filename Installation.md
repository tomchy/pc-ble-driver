...
Install conan the following command:

    pip install conan

The project requires the [ASIO package](https://bintray.com/bincrafters/public-conan/asio%3Abincrafters) from the bintray remote.

If the bintray remote has not been configured, you can do it with the following command:

    conan remote add bincrafters-public https://api.bintray.com/conan/bincrafters/public-conan
...
Then change to the root folder of the repository and issue the following commands:
    ...
    $ cd build
    > conan install ..
    > cmake -G "Unix Makefiles" <-DCMAKE_BUILD_TYPE=<build_type>> <-DARCH=<x86_32,x86_64>>" ..
    $ make
